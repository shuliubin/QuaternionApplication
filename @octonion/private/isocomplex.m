...
% ISOCOMPLEX    Construct a complex number from an octonion, with the same
%               modulus and argument as those of the octonion. The result
...
%               7-space, so that any direction in this hemisphere is taken
...
if nargin == 2
    if ~isa(a, 'octonion')
        error(['Second argument to private function isocomplex ' ...
               'must be an octonion (array).']);
    end
    if ~isreal(a)
        error(['Second argument to private function isocomplex ' ...
               'must be a real octonion (array).']);
    end
    if ~ispure(a) || any(any(abs(a) - 1 > eps))
        error(['Second argument to private function isocomplex ' ...
               'must be a pure unit octonion (array).']);
    end
...
if ~isreal(s) || ~isreal(m)
    error('QTFM:limitation', ...
          ['Private function isocomplex is undefined ' ...
           'for complex octonion arguments.'])
end;
...
% octonion with an axis perpendicular to the reference axis defined by
% the second parameter cannot be unambiguously converted to a complex
% number, since the octonion axis could be taken to be positive or
% negative when converted to the isomorphic complex number. The choice made
% here is to use the positive sense, so that the octonions for which a
...
% Note that this is not dependent on the argument of the octonion, but
% only on the relationship between the axis of the octonion q and the
% reference vector a. Steve Sangwine 6 March 2008.

% $Id: isocomplex.m,v 1.2 2016/06/15 13:19:45 sangwine Exp $
