function P = omega(A)
% OMEGA Computes the omega (left) adjoint of the octonion matrix A as in:
%
...
P = [phi(a), K4(-tao(b)); % K4 is a sub-function, see below.
     K4(phi(b)), tao(a)];
...
function Y = K4(X)
% This sub-function implements post-multiplication of 4-by-4 blocks of X by
% the matrix K4 = diag([1, -1, -1, -1]), which has the effect of negating
% all elements apart from those in the first column of each block. The
% method used is to copy and negate the whole array, then negate the 1st,
% 4th, etc columns so that their sign matches the original array.

Y = -X;
Y(:, 1:4:end-3) = -Y(:, 1:4:end-3);

end

% $Id: omega.m,v 1.2 2016/06/15 13:19:45 sangwine Exp $
