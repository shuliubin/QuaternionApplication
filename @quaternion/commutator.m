...
% term:
%
% 'diff' means the pure quaternion, c = ab - ba;
% 'prod' means the unit quaternion, c, that satisfies: abc = ba.
%
...
% between the product ab and ba, the other is a quaternion that multiplies
...
if strcmp(option, 'diff')
    ...
    % above). By definition this is a pure quaternion, so we take the
    % vector part to eliminate the scalar part which will be zero to within
    % rounding error.
...
elseif strcmp(option, 'prod')
    ...
    % This is the unit quaternion that multiplies the product of a and b to
...
% $Id: commutator.m,v 1.2 2016/06/15 13:19:43 sangwine Exp $
